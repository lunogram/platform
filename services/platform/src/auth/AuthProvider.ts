...
import App from '../app'
import Admin, { AuthAdminParams } from './Admin'
import { getAdminByEmail } from './AdminRepository'
import { generateAccessToken, OAuthResponse, setCookiesOauthToken } from './TokenRepository'
...
export default abstract class AuthProvider {
    ...
    webhook?(ctx: AuthContext): Promise<void>

    async loadAuthOrganization(ctx: AuthContext): Promise<{ organization: Organization, isNew: boolean }> {
    ...
    async login({ ...params }: AuthAdminParams, ctx: AuthContext, redirect?: string): Promise<OAuthResponse> {
        ...
        if (!admin) {
            const { organization, isNew } = await this.loadAuthOrganization(ctx)
