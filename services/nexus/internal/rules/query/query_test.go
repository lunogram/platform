...
func TestQueryBuilderBuildQuery(t *testing.T) {
	...
	tests := map[string]test{
		...
		"event with attributes": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ue.user_id FROM user_events ue JOIN events e ON e.id = ue.event_id WHERE e.name = $1 AND e.product_category = $2) e1 ON e1.user_id = u.id WHERE u.project_id = $3",
		...
		"complex frequency with nested jsonb": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT ue.user_id FROM user_events ue JOIN events e ON e.id = ue.event_id WHERE ue.created_at >= NOW() - $1::interval AND e.name = $2 AND (e.data->'product'->>'category')::text = $3 AND (e.data->'product'->>'price')::numeric >= $4 GROUP BY ue.user_id HAVING COUNT(*) >= $5) e1 ON e1.user_id = u.id WHERE u.project_id = $6",
