...
func TestQueryBuilderBuildCondition(t *testing.T) {
	...
	tests := map[string]test{
		...
		"in operator": {
			...
			wantSQL:  "u.project_id = $2 AND u.country = $1",
...
func TestQueryBuilderComplexRules(t *testing.T) {
	...
	tests := map[string]test{
		"complex multi-level rule": {
			...
			wantSQL:  "u.project_id = $8 AND (u.country = $1 AND (u.age >= $2 OR u.parent_consent = $3) AND (SELECT COUNT(*) FROM user_events e WHERE e.user_id = u.id AND e.project_id = $5 AND e.created_at >= NOW() - $4::interval AND e.name = $6) >= $7)",
...
func TestQueryBuilderNestedJSONBPaths(t *testing.T) {
	...
	tests := map[string]test{
		...
		"bracket notation with single quote in key": {
			...
			wantSQL:  "u.project_id = $2 AND (u.data->'user''s preference'->>'theme')::text = $1",
			wantArgs: []any{"dark", testProjectID},
			wantErr:  false,
		},
