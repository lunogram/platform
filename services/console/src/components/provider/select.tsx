    ...
    useEffect(() => {
        const fetchProviders = async () => {
            ...
            try {
                ...
                setProviders(result.filter(provider => provider.group === channel));

                if (providers.length > 0) {
                    const defaultProvider = result.find(provider => provider.is_default && provider.group === channel) ?? providers[0];
                    if (!value) {
