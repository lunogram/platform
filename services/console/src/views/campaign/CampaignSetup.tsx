...
const setupFormEmail = setupBaseFormSchema.extend(
  z.object({
      ...
      <Controller
        ...
        render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid} className="gap-2">
            <FieldLabel htmlFor="form-rhf-demo-name">Campaign Name</FieldLabel>
            <Input
              {...field}
...
function EmailSetup({ className, form }: EmailSetupProps) {
  ...
  return (
    <>
      <div
        className={cn(
          "bg-white border rounded-md shadow-sm w-full overflow-hidden",
          className
        )}
      >
...
export function UserSelection({
  ...
  const fetchUsers = useCallback(async () => {
    if (!projectId) return;
    const res = await api.users.search(projectId, {
      q: search,
      limit: 50,
    });
