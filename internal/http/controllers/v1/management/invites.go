...
type InviteController struct {
	...
	// mail and renderer are nil on a deployment that configures no mail
	// channel. Invites still work there -- they are discovered on the console's
	// invites page -- so a missing mailer costs the notification, not the
	// invite.
	mail     *mailer.Dispatcher
	renderer *mailer.Renderer
}

func NewInviteController(logger *zap.Logger, mgmt *management.State, engine *rbac.Engine, db *sqlx.DB, mail *mailer.Dispatcher, renderer *mailer.Renderer) *InviteController {
...
const (
	// defaultInviteTTL is used when the caller omits expires_in. The console
	// never sends one, so in practice this is how long every invite lives.
	defaultInviteTTL = 48 * time.Hour
...
func (srv *InviteController) CreateProjectInvite(w http.ResponseWriter, r *http.Request, projectID uuid.UUID) {
	...
	srv.notifyInvitee(ctx, invite, actorAdmin, ttl)
	json.Write(w, http.StatusCreated, invite.OAPI())
}

// notifyInvitee mails the invitation to the address it was sent to.
//
// The send is best effort and deliberately does not affect the response: the
// invite is already committed and is discoverable on the console's invites page
// without any mail at all, so a mailer that is unconfigured, or a project whose
// name will not load, must not turn a successful invite into an error.
func (srv *InviteController) notifyInvitee(ctx context.Context, invite *management.Invite, inviter *management.Admin, ttl time.Duration) {
	if srv.mail == nil || srv.renderer == nil {
		return
	}

	project, err := srv.mgmt.GetProject(ctx, invite.ProjectID, nil)
	if err != nil {
		srv.logger.Error("failed to load the project for an invite notification",
			zap.String("invite_id", invite.ID.String()), zap.Error(err))
		return
	}

	srv.mail.Dispatch(srv.renderer.ProjectInvite(invite.InviteeEmail, project.Name, inviterName(inviter), ttl))
}

// inviterName is who the invitation says it came from: the inviter's name when
// their account carries one, and their address otherwise. The address is a
// reasonable fallback because it is the only other thing the invitee can
// recognise them by.
func inviterName(admin *management.Admin) string {
	var parts []string
	for _, part := range []*string{admin.FirstName, admin.LastName} {
...
func (srv *InviteController) AcceptProjectInvite(w http.ResponseWriter, r *http.Request, inviteID uuid.UUID) {
	...
	if project.OrganizationID != nil {
		...
		// Accepting is a request to go and work on that project, and every
		// project read is scoped to the organization the admin is currently in.
		// Leaving them in their previous one sends them to a project the very
		// next request reports as not found. It is set even when the membership
		// already existed, because being a member of the owning organization is
		// not the same as currently being in it.
		if err := managementStore.SetActiveOrganization(ctx, adminID, orgID); err != nil {
			logger.Error("failed to activate the invited organization", zap.Error(err))
			oapi.WriteProblem(w, err)
			return
		}
	}
	...
	//
	// On a role upgrade the stale lower-role tuple is removed first. Otherwise
	// the role is SET rather than added: this branch also covers reviving a
	// membership that was removed earlier, where the soft-deleted row read as
	// absent and left us with no old role to revoke. Adding alone would leave
	// the previous membership's tuple in place, and the check would resolve that
	// one -- which is how an admin in Postgres ends up an editor to
	// authorization. Setting makes a re-accept reconcile the grant rather than
	// accumulate onto it.
	if roleUpgraded {
		err = access.UpdateProjectRole(ctx, srv.engine, adminID, invite.ProjectID, previousRole, invite.Role)
	} else {
		err = access.SetProjectRole(ctx, srv.engine, adminID, invite.ProjectID, invite.Role)
	}
	if err != nil {
