...
type basicVerifierEnv struct {
	verifier *Basic
	mgmt     *management.State
	orgID    uuid.UUID
}

func newBasicVerifierEnv(t *testing.T) *basicVerifierEnv {
	t.Helper()
	...
	return &basicVerifierEnv{
		verifier: NewBasic(mgmt, zaptest.NewLogger(t)),
		mgmt:     mgmt,
		orgID:    orgID,
	}
}
...
func (e *basicVerifierEnv) admin(t *testing.T, email, plain string, verified bool) (uuid.UUID, uuid.UUID) {
	t.Helper()
...
func TestBasicDriver(t *testing.T) {
	t.Parallel()
	assert.Equal(t, BasicDriver, NewBasic(nil, zaptest.NewLogger(t)).Driver())
}
