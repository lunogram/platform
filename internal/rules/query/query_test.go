...
func TestQueryBuilderNestedJSONBPaths(t *testing.T) {
	...
	tests := map[string]test{
		"single nested jsonb path": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $2 AND u.data @> jsonb_build_object('subscription', to_jsonb($1::text))",
		...
		"bracket notation with spaces": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $2 AND u.data @> jsonb_build_object('purchase agreement', to_jsonb($1::text))",
		...
		"mixed bracket and dot notation": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $2 AND u.data @> jsonb_build_object('purchase agreement', jsonb_build_object('value', to_jsonb($1::text)))",
		...
		"bracket notation with double quotes": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $2 AND u.data @> jsonb_build_object('user info', jsonb_build_object('name', to_jsonb($1::text)))",
			wantArgs: []any{"John", testProjectID},
			wantErr:  false,
		},
		"complex path with special characters": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $2 AND u.data @> jsonb_build_object('customer-id', jsonb_build_object('verified', to_jsonb($1::text)))",
		...
		"double nested jsonb path": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $2 AND u.data @> jsonb_build_object('subscription', jsonb_build_object('tier', to_jsonb($1::text)))",
		...
		"triple nested jsonb path": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $2 AND u.data @> jsonb_build_object('preferences', jsonb_build_object('notifications', jsonb_build_object('email', to_jsonb($1::text))))",
			wantArgs: []any{true, testProjectID},
			wantErr:  false,
		},
		...
		"nested jsonb in complex wrapper": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $3 AND (u.data @> jsonb_build_object('subscription', jsonb_build_object('plan', to_jsonb($1::text))) AND (u.data->'subscription'->>'seats')::numeric >= $2)",
		...
		"mixed regular and nested jsonb fields": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $3 AND (u.email ILIKE $1 AND u.data @> jsonb_build_object('company', jsonb_build_object('verified', to_jsonb($2::text))))",
...
func TestQueryBuilderBuildQuery(t *testing.T) {
	...
	tests := map[string]test{
		...
		"complex frequency with nested jsonb": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT ue.user_id FROM user_events ue JOIN events e ON e.id = ue.event_id WHERE ue.created_at >= NOW() - $1::interval AND e.name = $2 AND ue.data @> jsonb_build_object('product', jsonb_build_object('category', to_jsonb($3::text))) AND (ue.data->'product'->>'price')::numeric >= $4 GROUP BY ue.user_id HAVING COUNT(*) >= $5) e1 ON e1.user_id = u.id WHERE u.project_id = $6",
...
func TestQueryBuilderOrganizationRules(t *testing.T) {
	...
	tests := map[string]test{
		...
		"organization nested jsonb data": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $2 AND o.data @> jsonb_build_object('tier', to_jsonb($1::text))) e1 ON e1.user_id = u.id WHERE u.project_id = $3",
		...
		"organization user role": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $2 AND ou.data @> jsonb_build_object('role', to_jsonb($1::text))) e1 ON e1.user_id = u.id WHERE u.project_id = $3",
		...
		"combined organization and organization user with AND": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $3 AND (o.data @> jsonb_build_object('tier', to_jsonb($1::text)) AND ou.data @> jsonb_build_object('role', to_jsonb($2::text)))) e1 ON e1.user_id = u.id WHERE u.project_id = $4",
		...
		"combined organization and organization user with OR": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $3 AND (o.data @> jsonb_build_object('tier', to_jsonb($1::text)) OR ou.data @> jsonb_build_object('role', to_jsonb($2::text)))) e1 ON e1.user_id = u.id WHERE u.project_id = $4",
			wantArgs: []any{"gold", "admin", testProjectID, testProjectID},
			wantErr:  false,
		},
		"user attribute AND organization attribute": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $3 AND o.data @> jsonb_build_object('tier', to_jsonb($2::text))) e1 ON e1.user_id = u.id WHERE u.project_id = $4 AND u.email ILIKE $1",
		...
		"multiple organization conditions": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $3 AND (o.data @> jsonb_build_object('tier', to_jsonb($1::text)) AND (o.data->>'seats')::numeric >= $2)) e1 ON e1.user_id = u.id WHERE u.project_id = $4",
		...
		"user OR organization - uses UNION for true OR semantics": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT u.id AS user_id FROM users u WHERE u.project_id = $2 AND u.email ILIKE $1 UNION SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $4 AND o.data @> jsonb_build_object('tier', to_jsonb($3::text))) e2 ON e2.user_id = u.id WHERE u.project_id = $5",
		...
		"user OR organization_user - uses UNION for true OR semantics": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT u.id AS user_id FROM users u WHERE u.project_id = $2 AND u.email ILIKE $1 UNION SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $4 AND ou.data @> jsonb_build_object('role', to_jsonb($3::text))) e2 ON e2.user_id = u.id WHERE u.project_id = $5",
		...
		"nested user OR organization - uses UNION for true OR semantics": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT u.id AS user_id FROM users u WHERE u.project_id = $3 AND u.email ILIKE $2 UNION SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $5 AND o.data @> jsonb_build_object('tier', to_jsonb($4::text))) e2 ON e2.user_id = u.id WHERE u.project_id = $6 AND u.name = $1",
		...
		"user OR user - should work": {
			...
			wantSQL:  "SELECT u.id FROM users u WHERE u.project_id = $3 AND (u.email ILIKE $1 OR u.data @> jsonb_build_object('role', to_jsonb($2::text)))",
		...
		"nested org wrapper with parent - falls through to buildWrapper": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $2 AND o.data @> jsonb_build_object('tier', to_jsonb($1::text))) e1 ON e1.user_id = u.id JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $5 AND (ou.data @> jsonb_build_object('role', to_jsonb($3::text)) OR ou.data @> jsonb_build_object('role', to_jsonb($4::text)))) e2 ON e2.user_id = u.id WHERE u.project_id = $6",
...
func TestQueryBuilderOrganizationPathNormalization(t *testing.T) {
	...
	tests := map[string]test{
		...
		"wrapper with mixed org and org-user paths without data prefix": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $3 AND (o.data @> jsonb_build_object('tier', to_jsonb($1::text)) AND ou.data @> jsonb_build_object('admin', to_jsonb($2::boolean)))) e1 ON e1.user_id = u.id WHERE u.project_id = $4",
...
func TestQueryBuilderOrganizationEventWithOrConditions(t *testing.T) {
	...
	tests := map[string]test{
		"or with organization event and organization property": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN (SELECT oe.organization_id FROM organization_events oe JOIN events e ON e.id = oe.event_id WHERE oe.created_at >= NOW() - $1::interval AND e.name = $2 AND (oe.data->>'version')::numeric > $3 GROUP BY oe.organization_id HAVING COUNT(*) >= $4) matching_orgs ON matching_orgs.organization_id = ou.organization_id WHERE ou.data @> jsonb_build_object('admin', to_jsonb($5::boolean)) UNION SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $7 AND o.data @> jsonb_build_object('tier', to_jsonb($6::text))) e3 ON e3.user_id = u.id WHERE u.project_id = $8",
...
func TestQueryBuilderOrganizationWrapperEdgeCases(t *testing.T) {
	...
	tests := map[string]test{
		...
		"organization wrapper with member conditions and no property filters": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $2 AND TRUE AND ou.data @> jsonb_build_object('is_primary_contact', to_jsonb($1::boolean))) e1 ON e1.user_id = u.id WHERE u.project_id = $3",
		...
		"organization wrapper with property filter and member conditions": {
			...
			wantSQL:  "SELECT u.id FROM users u JOIN (SELECT DISTINCT ou.user_id FROM organization_users ou JOIN organizations o ON o.id = ou.organization_id WHERE o.project_id = $3 AND o.data @> jsonb_build_object('tier', to_jsonb($1::text)) AND ou.data @> jsonb_build_object('is_primary_contact', to_jsonb($2::boolean))) e1 ON e1.user_id = u.id WHERE u.project_id = $4",
