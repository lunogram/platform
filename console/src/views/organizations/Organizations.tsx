...
import { Plus, Building2, Search, ChevronLeft, ChevronRight, ArrowRight, Users, Settings } from 'lucide-react'
...
export default function Organizations() {
    ...
    const [result, , reload] = useResolver(
        useCallback(async () => {
            const { data } = await oapiClient.GET('/api/admin/projects/{projectID}/subjects/organizations', {
                params: {
                            ...
                            <TableRow>
                                <TableCell colSpan={3} className="h-32 text-center">
                                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                                        ...
                                        <p>{debouncedQuery ? t('no_organizations_found') : t('no_organizations_yet')}</p>
                                        {!debouncedQuery && (
                                            <Button
                                                variant="outline"
                                                size="sm"
            ...
            <div className="group relative overflow-hidden rounded-lg bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border p-6">
                <div className="relative z-10 max-w-md">
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                        {t('sync_organizations_description', 'Keep your organizations in sync with your system by using the API to create and update them automatically.')}
                    </p>
                    <Button 
                        variant="link" 
                ...
                <div className="absolute -right-6 -bottom-6 flex gap-4">
                    <div className="flex h-20 w-20 items-center justify-center rounded-xl bg-primary/10 rotate-12 transition-all duration-500 ease-out group-hover:rotate-6 group-hover:-translate-y-2 group-hover:bg-primary/15">
                        <Building2 className="h-10 w-10 text-primary/40 transition-all duration-500 group-hover:text-primary/60 group-hover:scale-110" strokeWidth={1.25} />
                    </div>
            ...
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {t('create_organization_description', 'Create a new organization to group users together.')}
                        </DialogDescription>
                    </DialogHeader>
