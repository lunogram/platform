...
export default function OrganizationDetailAttrs() {
    ...
    const handleSave = async () => {
        setIsSaving(true)
        try {
            const { data: responseData, error } = await oapiClient.PATCH('/api/admin/projects/{projectID}/subjects/organizations/{organizationID}', {
                params: {
                    path: {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-base font-medium">{t('custom_attributes', 'Custom attributes')}</h2>
                    <p className="text-sm text-muted-foreground mt-0.5">
                        {t('organization_data_description', 'Store custom metadata and attributes for this organization')}
                    </p>
                </div>
                {isDirty && (
            ...
            <AttributeEditor
                ...
                emptyDescription={t('no_organization_data_description', 'Add custom data to store additional information about this organization.')}
            />
        </div>
    )
}
