...
export default function OrganizationDetailEvents() {
    ...
    const [result] = useResolver(
        useCallback(async () => {
            const response = await oapiClient.GET('/api/admin/projects/{projectID}/subjects/organizations/{organizationID}/events', {
                params: {
                    path: {
                                ...
                                <TableCell colSpan={4} className="h-48">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-sm text-muted-foreground max-w-xs text-center">
                                            {t('no_events_description', 'Events will appear here when activity occurs')}
                                        </p>
                                    </div>
                                </TableCell>
                            </TableRow>
                            ...
                            events.map((event) => {
                                ...
                                return (
                                    <React.Fragment key={event.id}>
                                        ...
                                        >
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-3">
                                                    <div
                                                        ...
                                                        style={{ backgroundColor: getRandomColor(event.name) }}
                                                    >
                                            ...
                                            <TableCell className="text-muted-foreground">
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    {formatDate(preferences, event.created_at, 'Pp')}
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                {hasData ? (
                                        ...
                                        {/* Expanded Row */}
                                        {isExpanded && (
                                            <EventExpandedRow key={`${event.id}-expanded`} event={event} />
                                        )}
                                    </React.Fragment>
                                )
                            })
