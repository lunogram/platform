...
export default function OrganizationDetailMembers() {
    ...
    const [editedMemberData, setEditedMemberData] = useState<Record<string, Record<string, unknown>>>({})
    ...
    const [result, , reload] = useResolver(
        useCallback(async () => {
            const { data } = await oapiClient.GET('/api/admin/projects/{projectID}/subjects/organizations/{organizationID}/users', {
                params: {
                    path: {
    ...
    const addMember = async (user: UserType) => {
        await oapiClient.POST('/api/admin/projects/{projectID}/subjects/organizations/{organizationID}/users', {
            params: {
                path: {
                    projectID: project.id,
                    organizationID: organization.id,
    ...
    const removeMember = async () => {
        ...
        setIsRemoving(true)
        try {
            await oapiClient.DELETE('/api/admin/projects/{projectID}/subjects/organizations/{organizationID}/users/{userID}', {
                params: {
                    path: {
                        projectID: project.id,
                        organizationID: organization.id,
    ...
    const getMemberOrganizationData = (member: OrganizationMember): Record<string, unknown> => {
        return editedMemberData[member.id] ?? (member.organization_data as Record<string, unknown>) ?? {}
    }
                    ...
                    <TableBody>
                        {members === undefined ? (
                            Array.from({ length: 5 }).map((_, i) => (
                                <TableRow key={i}>
                                    <TableCell className="p-0 pl-3"><Skeleton className="h-4 w-4" /></TableCell>
                                    <TableCell>
                                ...
                                <TableCell colSpan={5} className="h-48">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <p className="font-medium mb-1">
                                            {debouncedQuery ? t('no_members_found') : t('no_members_yet')}
                                        </p>
                                        {!debouncedQuery && (
                                            <Button
                                                variant="outline"
                                ...
                                return (
                                    <React.Fragment key={member.id}>
                                        ...
                                        >
                                            ...
                                            <TableCell>
                                                {member.external_id ? (
                                                    <code className="text-sm text-muted-foreground">{member.external_id}</code>
                                                ) : (
                                        ...
                                        {isExpanded && (
                                            <MemberExpandedRow
                                                ...
                                                onDataChange={(data) => handleMemberDataChange(member.id, data)}
            ...
            <Dialog open={memberToRemove !== null} onOpenChange={(open) => !open && setMemberToRemove(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {t('remove_member_warning', 'Are you sure you want to remove this member from the organization?')}
                        </DialogDescription>
                    </DialogHeader>
                    {memberToRemove && (
                        <div className="py-4">
                            <div className="flex items-center gap-3 p-3 rounded-lg bg-muted">
                                ...
                                <div>
                                    <p className="font-medium">{getMemberDisplayName(memberToRemove)}</p>
                                    <p className="text-sm text-muted-foreground">
