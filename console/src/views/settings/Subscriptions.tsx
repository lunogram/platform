                        ...
                        {!result ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={4} className="h-32 text-center">
                                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                                        ...
                                        <p>{searchQuery ? t('no_results') : t('no_subscriptions_yet', 'No subscriptions yet')}</p>
                                        {!searchQuery && (
                                            <Button
                                                variant="outline"
                                                size="sm"
                            ...
                            subscriptions.map((sub) => (
                                ...
                                >
                                    ...
                                    <TableCell>
                                        <Badge variant="secondary">{snakeToTitle(sub.channel)}</Badge>
                                    </TableCell>
                ...
                {subscriptions.length > 0 && (
                    <div className="flex items-center justify-between border-t px-4 py-3">
                        <p className="text-sm text-muted-foreground">
                            {subscriptions.length} {subscriptions.length === 1 ? t('subscription', 'subscription') : t('subscriptions')}
                        </p>
                    </div>
                )}
            </div>
        ...
        <Dialog open={!!editing} onOpenChange={(open) => { if (!open) onClose() }}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {isUpdate
                            ? t('update_subscription_description', 'Update the subscription details.')
                            : t('create_subscription_description', 'Create a new subscription topic.')
                        }
                    </DialogDescription>
                </DialogHeader>
                    ...
                    {!isUpdate && (
                        <div className="grid gap-2">
                            ...
                            <Select
                                ...
                                onValueChange={(val) => form.setValue('channel', val as Subscription['channel'])}
                            >
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    {['email', 'push', 'text', 'webhook'].map((channel) => (
