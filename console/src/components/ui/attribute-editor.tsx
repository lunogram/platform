...
function valueToTree(value: unknown, key: string = ''): TreeNode {
    ...
    if (typeof value === 'boolean') {
        return { id, key, type: 'boolean', value: value ? 'true' : 'false', children: [], expanded: true }
    }
...
function treeToValue(node: TreeNode): unknown {
    switch (node.type) {
        ...
        case 'boolean': return node.value === 'true'
        case 'array': return node.children.map(child => treeToValue(child))
        case 'object': {
...
function TreeRow({ node, depth, isArrayItem, index, onUpdate, onDelete, onAddChild }: TreeRowProps) {
            ...
            <div className="flex items-center py-1.5">
                ...
                {isContainer && (
                    <button
                        ...
                        aria-label={node.expanded ? t('collapse', 'Collapse') : t('expand', 'Expand')}
                    ...
                    <div className="inline-flex items-center">
                        ...
                        {isContainer ? (
                            ...
                            <>
                                ...
                                {/* Value input */}
                                {node.type === 'boolean' ? (
                                    <Select value={node.value} onValueChange={(value) => handleUpdate({ value })}>
...
export function AttributeEditor({
    ...
    const notifyChange = useCallback((newNodes: TreeNode[]) => {
    ...
    const findAndUpdate = (nodes: TreeNode[], id: string, updates: Partial<TreeNode>): TreeNode[] => {
    ...
    const findAndAddChild = (nodes: TreeNode[], parentId: string): TreeNode[] => {
        return nodes.map(node => {
            if (node.id === parentId) {
                const newChild = createNode(node.type === 'array' ? String(node.children.length) : '')
    ...
    return (
        <div className={cn("border rounded-lg overflow-hidden", className)}>
            ...
            <div className="flex items-center justify-between px-3 py-2 bg-muted/30 border-b">
                ...
                <div className="flex items-center rounded-md bg-muted p-0.5" role="tablist" aria-label={t('editor_mode', 'Editor mode')}>
                    <button
                ...
                isEmpty ? (
                    <div className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-xs text-muted-foreground text-center max-w-xs mb-4">
                            {emptyDescription ?? t('add_attributes_description', 'Add custom data to store additional information.')}
                        </p>
                        <Button variant="outline" size="sm" onClick={addRootNode} className="shadow-none">
