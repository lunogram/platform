# Segment
Lunogram currently supports both ingesting events from Segment and sending events to Segment. Follow the instructions for each below.

## Outbound
To setup Lunogram as a source for events going into Segment, you first need to create a Source in Segment. 
...
4. The source screen should present you with a `Write Key` which is needed to setup the intergration on Lunogram. Copy this for later.
5. Navigate to Lunogram and go to `Settings -> Integrations`
...
## Inbound
There is currently no Destination in Segment for Lunogram, however Lunogram does have a Segment compatible endpoint that can be used for webhooks. 

1. Open Lunogram and go to `Settings -> API Keys`
...
10. For the first section, pick `Event Type` as the condition and `Track` as the event type. Lunogram supports `Track`, `Identify` and `Alias` events, so select all three as separate conditions to receive all data.
11. Under mappings, enter the following information:
    ...
    - **Method**: POST
    - **Headers**: Set the key as `Authorization` and the value to `Bearer API_KEY` where `API_KEY` is the key you created in Lunogram
    - **Data**: Leave as is
    - **Enable Batching**: Set to true
12. Save the destination and enable to start sending events into Lunogram!.