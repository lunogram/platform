...
3. In general for Lunogram you will want the flexibility of sending emails from any address, so we recommend picking the `Domain` entity type.
...
12. Open a new window and go to your Lunogram project settings
...
## Inbound
Email sending is not the only important part, you also need to keep track of things like email opens, clicks, unsubscribes, bounces and complaints. Lunogram automatically takes care of opens, clicks and unsubscribes for you, but bounces and complaints require notifications from SES.
...
6. For Protocol, select `HTTPS` and enter the Lunogram SES unsubscribe URL for your provider (which can be found on the provider details screen)
7. Hit save
