...
They live in the `webhook.outbound` section of the [node configuration](/docs/settings/configuration):
...
A referenced variable that is not set is a startup error. Lunogram will not quietly send an empty API key. Quote your placeholders — `"${NAME}"` — so a `#` inside a secret is not read as the start of a YAML comment. See [Configuration](/docs/settings/configuration#secrets) for the full rules.
...
`body` is a `file://` reference, a `base64://` payload or an inline snippet. Relative `file://` paths resolve against the directory holding the configuration file, so a config and its templates travel together; `base64://` is the form to use when the template arrives from an environment variable, because it is the only one that survives `${}` interpolation intact. See [Configuration](/docs/settings/configuration#values-that-do-not-fit-on-one-line).
