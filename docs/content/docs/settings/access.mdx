...
Each project can restrict which event names untrusted clients may emit. It applies to **public API keys** and **verified end users** (trusted-issuer and session requests) calling `POST /api/client/events` and the organization events endpoint; trusted **secret** keys are exempt.
