...
Every value can also be given as `file://` or `base64://`. See [Configuration](/docs/settings/configuration#values-that-do-not-fit-on-one-line) for when to use which, and for what happens when a reference is broken.
