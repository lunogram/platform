...
`base64://` earns its place on large values. An HTML email is a few hundred lines of markup: written inline it has to become a YAML block scalar and stay correctly indented, and passed through an environment variable it has to survive whatever the surrounding shell, ConfigMap or secret store does to line breaks. Base64 makes it one opaque token that none of those can reshape:
...
Nothing forces you to use it — a multi-line value expands correctly on its own, and `file://` is the better answer once a template is big enough that you want to edit it.
