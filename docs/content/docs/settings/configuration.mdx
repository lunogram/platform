...
mail:
  templates:
    layout:
      html: base64://${MAIL_LAYOUT_HTML}
```

```bash
MAIL_LAYOUT_HTML="$(base64 < layout.html)"
```

The full HTML lives under `templates.layout`, which is the template allowed to emit markup. Per-message keys (`password_reset` and friends) hold the copy — subject, heading, body, action label, footer — not a document. See [Sending mail](/settings/mail#overriding-the-messages).
...
See [Sign-in](/settings/sign-in) for the `auth` section, [Sending mail](/settings/mail) for `mail`, and [Webhooks](/settings/webhooks) for `webhook.outbound`.
