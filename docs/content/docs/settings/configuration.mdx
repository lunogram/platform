...
The full HTML lives under `templates.layout`, which is the template allowed to emit markup. Per-message keys (`verify_email` and friends) hold the copy — subject, heading, body, action label, footer — not a document. See [Sending mail](/docs/settings/mail#overriding-the-messages).
