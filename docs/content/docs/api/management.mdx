...
Management endpoints authenticate via Bearer token using an API key. The level of access depends on the key's role. See [Access](/settings/access) for a full breakdown of available roles and their permissions.
