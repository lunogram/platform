---
title: Client API
description: Ingestion API endpoints used by client SDKs for tracking users and events
---
...
Authentication to client endpoints is handled via public bearer API keys. These API keys allow for ingestion only of data and only on select endpoints. You can [generate a new public key](settings/api-keys) at any time in the portal and we recommend using a separate key per platform.
...
- `anonymous_id` (string) - The anonymous identifier to merge from
- `external_id` (string) - The external identifier to merge to
...
- `external_id` (string) - Your internal identifier for the user
...
- `locale` (string, optional) - The locale of the user used for language and formatting (i.e `es` or `en`)
- `data` (object, optional) - Custom properties to associate with the user
...
- `anonymous_id` (string) - Anonymous identifier for the user
- `external_id` (string) - Your internal identifier for the user
...
- `app_build` (string) - App build number (i.e. 465)
- `app_version` (string) - App version string (i.e. 1.2.4)
...
- `anonymous_id` (string, optional) - Anonymous identifier for the user
- `external_id` (string, optional) - Your internal identifier for the user
- `data` (object, optional) - Custom properties to associate with the event
