...
<Callout type="info">
  **Timezone auto-conversion**: Values in the `timezone` column are automatically
  ...
  GMT/UTC offsets like `GMT-5` or `+02:00` are resolved to a best-match IANA
  timezone name (for example, `America/New_York`, `America/Los_Angeles`).
  IANA names like `America/New_York` are used as-is. Note that abbreviation and
  offset conversion is heuristic/lossy: inputs may map to a region-based timezone
  that observes DST, so a fixed offset may not be preserved year-round.
</Callout>
