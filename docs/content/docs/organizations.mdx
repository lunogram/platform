...
Create organizations through the client API. If an organization with the same `external_id` already exists, the call updates it instead.
...
Delete an organization by its `external_id`. This also removes all memberships.
...
Add a user to an organization using both external IDs:
...
| Field                      | Type   | Required | Description                             |
| -------------------------- | ------ | -------- | --------------------------------------- |
| `organization_external_id` | string | Yes      | The organization's external ID          |
| `user_external_id`         | string | Yes      | The user's external ID                  |
