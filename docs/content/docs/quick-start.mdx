...
Lunogram is now running at [http://localhost:8080](http://localhost:8080).

<Callout type="warn">
  `AUTH_JWT_SECRET` signs admin session tokens and has no default. Generate your
  own private value of at least 32 bytes — anyone who knows the secret can mint
  a valid admin session against your deployment. Rotating it signs everyone out.
</Callout>
