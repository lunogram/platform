...
<Callout type="info">
  **Timezone auto-conversion**: Values in the `timezone` column are automatically
  ...
  GMT/UTC offsets like `GMT-5` or `+02:00` are resolved to a best-fit IANA
  timezone (for example, `America/New_York`, `America/Los_Angeles`). IANA names
  like `America/New_York` are used as-is. Note that abbreviation/offset
  conversion is heuristic and lossy: a resolved regional timezone may observe
  daylight saving time and therefore may not preserve the original fixed offset
  year-round.
</Callout>
