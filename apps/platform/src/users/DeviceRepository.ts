...
export const markDevicesAsPushDisabled = async (projectId: UUID, tokens: string[], trx?: Transaction): Promise<void> => {
    await Device.update(qb => qb.whereIn('token', tokens).where('project_id', projectId), { token: null }, trx)
}

export const userHasPushDevice = async (projectId: UUID, userId: UUID, trx?: Transaction): Promise<boolean> => {
...
export const getPushDevicesForUser = async (projectId: UUID, userId: UUID, trx?: Transaction): Promise<PushDevice[]> => {
